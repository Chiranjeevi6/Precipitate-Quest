...
var questions_needed = 8    # Minimum questions needed for level completion
var max_questions = 10      # Maximum questions per level
var current_question = 0    # Track current question number
...
func correct():
	...
	$QNo.text = "QNo: " + str(current_question + 1)
	$PointsLabel.text = "Points: " + str(points)
	$correct.play()
	await get_tree().create_timer(2).timeout
	...
	if questions_correct >= questions_needed:
		end_level()
	elif current_question < max_questions - 1:
		next_question()
	else:
		end_level()
...
func end_level(failed=false):
	if failed:
		get_tree().change_scene_to_file("res://level_failed.tscn")
	else:
		print("Level complete!")
		FirebaseManager.saveHighScore(str(curr_scene.to_lower()), points)
...
# Additional button functions remain the same
...
var hint_dict = {
	...
	"Mg²⁺ Cl⁻" : "Magnesium ions are removed from seawater before the seawater is desalinated. Choose the compound that you can add to seawater to remove magnesium ions.",
	"CrO₄²⁻ K⁺": "You want to make some zinc chromate to use as a primer when painting metal surfaces. Choose the compound to add to form zinc chromate."
}